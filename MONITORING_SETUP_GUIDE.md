...
│ (Sistema)       │    │ (Base de Datos) │    │   Email/Slack   │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
docker-compose up -d prometheus grafana node-exporter mysqld-exporter
...
1. **Métricas de la Aplicación PHP**: http://localhost:80/metrics.php
...
1. **Editar AlertManager** (`monitoring/prometheus/alertmanager.yml`):
```yaml
global:
